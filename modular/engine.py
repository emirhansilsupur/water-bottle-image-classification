...
def train_step(model, dataloader, loss_fn, optimizer, acc_score, fbeta_score, device):
    """
    ...
    - acc_score: The accuracy score function to use.
    ...
    - train_acc: The average accuracy score over the entire training set for this epoch.
    """
    model.train()
    train_loss, train_acc, train_fbeta_score = 0, 0, 0
    ...
    return train_loss, train_acc, train_fbeta_score


def test_step(model, dataloader, loss_fn, acc_score, fbeta_score, device):
    """
    ...
    Args:
        ...
        acc_score (callable): The accuracy score function used to calculate the accuracy score.
        device (str): The device to use for the evaluation (e.g. "cpu", "cuda").

    Returns:
        Tuple[float, float, float]: The average test loss, F-beta score, and accuracy score over the test dataset.
    """
    model.eval()
    test_loss, test_acc, test_fbeta_score = 0, 0, 0
    ...
    return test_loss, test_acc, test_fbeta_score
...
def train(
    ...
    - train_acc: the average accuracy score for the epoch using the training set
    ...
    - test_acc: the average accuracy score for the epoch using the validation set
    """
    results = {
        "train_loss": [],
        "train_acc": [],
    ...
    for epoch in tqdm(range(epochs)):
        ...
        test_loss, test_acc, test_fbeta_score = test_step(
            model, test_dataloader, loss_fn, acc_score, fbeta_score, device
        )
        # scheduler.step()
        ...
        print(
            ...
            f"test_acc: {np.round(test_acc.item(),4)*100}% | "
